 ...
 * 如'02-02'转化为flag
 * @param date
 * @param orderTimeLimitEnd
 * @param orderTimeLimitESpanTime
 * @returns {number}
 */
const dateToFlag = (date, orderTimeLimitEnd, orderTimeLimitESpanTime) => {
  const selectedDate = moment(date, 'MM-DD')

  const today = moment(moment().format('MM-DD'), 'MM-DD')
  const isCrossYear = moment(moment().format('MM-DD'), 'MM-DD').isAfter(moment(date, 'MM-DD')) // 是否跨年了
  const now = moment()
  const month = date.split('-')[0]
  const day = date.split('-')[1]
  let flagDate
  const orderLimitEndTime = convertDateString(dateFullFormat(moment()) + ' ' + orderTimeLimitEnd)

  if (isCrossYear) {
    const days = today.diff(selectedDate, 'days')

    const dayCount = moment().endOf('y').dayOfYear()

    return dayCount - days
  }

  if (orderTimeLimitESpanTime === '1' && now < moment(orderLimitEndTime, 'YYYY/MM/DD HH:mm').valueOf()) {
    // 当前日期减一天
 ...
 * @returns {Array}
 */
const receiveDateGeneratorNew = (sSpanTime, eSpanTime, receiveEndSpan, rEnd, orderTimeLimitEnd) => {
  const now = moment().valueOf()
  const dateObj = {}
  const oneDaySpan = 24 * 60 * 60 * 1000
  const orderLimitEndTime = convertDateString(dateFullFormat(moment()) + ' ' + orderTimeLimitEnd).split(' ')[1]

  let endTime = Number(eSpanTime)

  let startTime = Number(sSpanTime)
  ...
  if (Number(receiveEndSpan) === 1) {
    ...
    endTime = (sSpanTime === '0' && time < orderLimitEndTime) ? endTime : endTime + 1
  }
 ...
 * @param orderTimeLimitEnd  今天下单截止时间
 * @returns {Array}  由形式为 '12-19 22：30' 的元素组成的数组
 */
const receiveTimeSpanGeneratorPreSell = (rStart, rEnd, startFlag, endFlag, receiveTimeSpan, receiveEndSpan, orderTimeLimitEnd) => {
  const dateArr = _.toArray(receiveDateGeneratorNew(startFlag, endFlag, receiveEndSpan, rEnd, orderTimeLimitEnd))
  const timeSpan = receiveTimeSpan * 60 * 1000
  const oneDaySpan = 24 * 60 * 60 * 1000
  const dateTimeArr = []
  ...
  for (let i = 0; i < dateArrLength; i++) {
    ...
    const startHour = rStart.slice(0, 2)
    const startMin = rStart.slice(-2)
    const endHour = rEnd.slice(0, 2)
    const endMin = rEnd.slice(-2)
    ...
    if (i === 0) {
      ...
      const endIndex = _.indexOf(timeArr, rEnd)
      if (endIndex > -1) {
      ...
      if (receiveEndSpan + '' === '1') {
        ...
        if (receiveTimeSpan === '60' && rStart.split(':')[1] === '30') {
        ...
        const startIndex = _.indexOf(timeArrTemp, rStart)
        const endIndex = _.indexOf(timeArrTemp, rEnd)
...
const getCircleDateTime = (targetDateTime, start, end, receiveEndSpan, receiveTimeSpan) => {
  ...
  const span = receiveTimeSpan * 60 * 1000
